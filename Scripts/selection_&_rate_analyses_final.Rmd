...
```{r source_functions_&_libraries, results='hide', echo=FALSE, message=FALSE}
rm(list=ls())
...
```{r read_in_data, echo=TRUE, results='asis', message=TRUE}
...
Next we checked the data for outliers and oddities that might indicate an error in data entry or similar. We find no obvious errors, thought there are 2 rates measures that are visual outliers. These do not appear to be in error, but are both from the same study, which was one of the experimental evolution studies. Though experimental/natural evolution is not a very strong predictor for rate ( `r Rsq( lm( response$absRate ~ response$NatExp ) )` ), this underlines that we need to account for this effect in our models.

```{r data_hygiene, results='hide', fig.keep='none', echo=FALSE}
...
```{r summary_information_for_table, echo=FALSE}
...
```{r model_selection_for_rates_Animals, echo=FALSE, message=FALSE, results='hide', warning=FALSE, fig.keep='none'}
...
```{r plot_results_Animals, echo=FALSE, results='hide', fig.keep='high'}
...
```{r model_selection_for_Plants, echo=FALSE, results='hide', message=FALSE, fig.keep='none', warning=FALSE}
...
```{r results_plants_&_animals, results='hide', echo=FALSE, fig.keep='high'}
...
```{r read_in_selection_data, echo=FALSE, results='hide'}
...
```{r selection_data_hygiene, echo=FALSE, message=FALSE}
names( select )
...
```{r modelling_beta, results='hide', echo=FALSE, fig.keep='none'}
...
This analysis is perhaps not the most suitable for our purposes -- we can see that some traits seen to expereince more variable linear selection gradients in plants than in animals, but the estimates are hovering close to zero because there are both positive and negative gradients in the dataset.

```{r plotting_results_for_beta, echo=FALSE, message=FALSE, fig.keep='high'}
...
```{r modelling_absolute_beta, results='hide', echo=FALSE, fig.keep='none' }
...
```{r plotting_results_for_absolute_beta, echo=FALSE, message=FALSE, fig.keep='high'}
...
Though only the analysis of linear selection gradients (betas) was included in the paper, we also tabulated quadratic selection gradients, which measure potentially stabilising or disruptive selection. These gradients had been estiamted less frequently, so there was a smaller dataset to work with.
...
```{r modelling_gamma+, results='hide', echo=FALSE, fig.keep='last'}
...
```{r modelling_gamma-, results='hide', echo=FALSE, fig.keep='last', warning=FALSE}
select_G <- subset( select, select$Gamma_SE > 0 )
